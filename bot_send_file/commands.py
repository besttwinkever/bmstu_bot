...
def send_file_command(call):
    ...
    for dis in Discipline.objects.all():
        ...
        if any(user_g in applicable for user_g in list(map(lambda g: g.name, user.user.groups.all()))):
            markup.add(telebot.types.KeyboardButton(text=dis.name))
...
def handle_discipline_input(message, user):
    ...
    if Discipline.objects.filter(name=discipline, teachers__username__in=[user]).count() == 0:
        bot.send_message(
            text="Нет доступных дисциплин",
            chat_id=message.chat.id,
            reply_markup=telebot.types.ReplyKeyboardRemove()
        ...
        for submission in SubmissionType.objects.filter(discipline__name=discipline):
                markup.add(telebot.types.KeyboardButton(text=submission.name))
        bot.delete_message(chat_id=telegram_id, message_id=sent_msg.message_id)
        if len(markup.keyboard) > 0:
            bot.send_message(
            ...
            bot.send_message(
                text="Нет доступных типов вложений",
                chat_id=message.chat.id,
                reply_markup=telebot.types.ReplyKeyboardRemove()
            )
...
def handle_submission_type_input(message, user, discipline):
    ...
    if SubmissionType.objects.filter(discipline__name=discipline, discipline__teachers__username__in=[user], name=submission_type).count() == 0:
        bot.send_message(
            text="Нет доступных типов вложений",
            chat_id=message.chat.id,
            reply_markup=telebot.types.ReplyKeyboardRemove()
